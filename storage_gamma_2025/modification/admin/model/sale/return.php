...
class ModelSaleReturn extends Model {
	public function addReturn($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "return_cost` SET return_id = '" . (int)$return_id . "', order_id = '" . (int)$data['order_id'] . "',  cost = '" . (float)$data['cost'] . "'");
            
		return $this->db->getLastId();
		
		 $this->load->model('extension/module/hb_return_email');
		$data['email_type']       = 'admin_alert';
        $this->model_extension_module_hb_return_email->add_return_email($data);  
	}

	public function editReturn($return_id, $data) {

		$return_cost_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "return_cost WHERE return_id = '" . (int)$return_id . "'");
		
		if ($return_cost_query->num_rows) {
			$this->db->query("UPDATE `" . DB_PREFIX . "return_cost` SET return_id = '" . (int)$return_id . "', order_id = '" . (int)$data['order_id'] . "',  cost = '" . (float)$data['cost'] . "' WHERE return_id = '" . (int)$return_id . "'");
		} else {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "return_cost` SET return_id = '" . (int)$return_id . "', order_id = '" . (int)$data['order_id'] . "',  cost = '" . (float)$data['cost'] . "'");
		}  
	...
	public function deleteReturn($return_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "return_cost` WHERE `return_id` = '" . (int)$return_id . "'");
            
		$this->db->query("DELETE FROM `" . DB_PREFIX . "return_history` WHERE `return_id` = '" . (int)$return_id . "'");
	}


	public function getReturnCost($return_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "return_cost WHERE return_id = '" . (int)$return_id . "'");
	...
	public function addReturnHistory($return_id, $return_status_id, $comment, $notify) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "return_history` SET `return_id` = '" . (int)$return_id . "', return_status_id = '" . (int)$return_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape(strip_tags($comment)) . "', date_added = NOW()");
                    ...
                    $data['return_history_id'] 	= $this->db->getLastId();
                    $data['return_id'] 			= $return_id;
					$data['comment'] 			= $comment;
                    
            		$this->load->model('extension/hbapps/order_return');
            		
            		if ($notify){
            		    $this->model_extension_hbapps_order_return->return_update_email($data);
            		}
