...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$release_status_id = $product['release_status_id'];
		$distributor_status_id = $product['distributor_status_id'];	
		$video = $product['video'];	
		...
		$sql .= "`tax_class_id`,`viewed`,`length`,`width`,`height`,`length_class_id`,`sort_order`,`subtract`,`minimum`,`date_release`,`release_status_id`, `distributor_status_id`, `video`, `product_rating`) VALUES ";
		...
		$sql .= "$tax_class_id,$viewed,$length,$width,$height,'$length_class_id','$sort_order','$subtract','$minimum',";
		$sql .= ($date_release=='NOW()') ? "$date_release," : "'$date_release',";
		$sql .= "$release_status_id, $distributor_status_id,'$video','$product_rating');";
		$this->db->query($sql);
		foreach ($languages as $language) {
			$language_code = $language['code'];
			$language_id = $language['language_id'];
        ...
        if (!empty($video)) {
            ...
            if (strpos($video, 'youtube.com') !== false) {
                // Extract the video ID and build the embeddable URL
                ...
                // If it's already in embed format or another URL type
                $embed_url = $this->db->escape($video);
            }
        
            // Prepare the content with the video
            ...
            // First, delete any existing entry with the same module_name
            ...
            // Prepare the SQL insert query
        					$sql = "INSERT INTO " . DB_PREFIX . "journal3_module (`module_id`, `module_type`, `module_name`, `module_data`)
        							VALUES (NULL, 'product_tabs', '" . $this->db->escape($module_name) . "', '" . $this->db->escape($module_data) . "')";
        
        					// Execute the query
        					$this->db->query($sql);
        } else {
            // Skip the insertion if no video data is provided
            // Optionally, you can log this or handle it in some way
        }
        ...
        if (isset($stock_status_id) && $stock_status_id == 15  && $quantity != 0) {
            ...
            if ($query->num_rows) {
                ...
                if (isset($moduleData['general']['filter']['products']) && is_array($moduleData['general']['filter']['products'])) {
                    ...
                    if (($key = array_search((string)$product_id, $products)) !== false) {
                        unset($products[$key]);
                        $moduleData['general']['filter']['products'] = array_values($products);
            ...
            // Step 2: Add the product to the 'Coming Soon' module if not present
            ...
            // Step 3: Handle category assignment for stock status 15
            $c_id = 53; // Preorder category
            $categories_to_remove = [52, 54, 59];
            $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id IN (" . implode(',', $categories_to_remove) . ")");
        
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id = '" . (int)$c_id . "'");
            if ($query->num_rows == 0) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ('" . (int)$product_id . "', '" . (int)$c_id . "')");
            }
        } elseif (isset($stock_status_id) && $stock_status_id == 13  && $quantity != 0) {
            // product label -- On Reorder start
            $query = $this->db->query("SELECT `module_data` FROM `oc_journal3_module` WHERE `module_name` = 'Coming Soon'");
            if ($query->num_rows) {
                $moduleData = json_decode($query->row['module_data'], true);
            ...
            // Step 3: Handle category assignment for stock status 13
            $c_id = 53; // Preorder category
            $categories_to_remove = [52, 54, 59];
            $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id IN (" . implode(',', $categories_to_remove) . ")");
            ...
            // Remove product from both modules if stock_status_id doesn't match
            foreach (['Coming Soon', 'On Reorder'] as $module_name) {
                $query = $this->db->query("SELECT `module_data` FROM `oc_journal3_module` WHERE `module_name` = '" . $this->db->escape($module_name) . "'");
                if ($query->num_rows) {
                    $moduleData = json_decode($query->row['module_data'], true);
                    ...
                    if (isset($moduleData['general']['filter']['products']) && is_array($moduleData['general']['filter']['products'])) {
                        ...
                        if (($key = array_search((string)$product_id, $products)) !== false) {
                            ...
                            $updatedModuleData = json_encode($moduleData, JSON_UNESCAPED_SLASHES);
                            $this->db->query("UPDATE `oc_journal3_module` SET `module_data` = '" . $this->db->escape($updatedModuleData) . "' WHERE `module_name` = '" . $this->db->escape($module_name) . "'");
                        }
                    }
                }
            }
	...
	protected function moreProductCells( $i, &$j, &$worksheet, &$product ) {
		return;
			...
			protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array(), $exclude_qsd ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			//05-02-2022 exp_imp_upd start
			if ($exclude_qsd === "exclude_qsd") {
    	        ...
    	        $query = $this->db->query( "SELECT date_available FROM `".DB_PREFIX."product` WHERE product_id='".$product_id."'" );
    			if ($query->num_rows > 0) {
    			    $date_available = $query->rows[0]['date_available'];
    			}
			
    			$query = $this->db->query( "SELECT stock_status_id FROM `".DB_PREFIX."product` WHERE product_id='".$product_id."'" );
    			if ($query->num_rows > 0) {
    			    $stock_status_id = $query->rows[0]['stock_status_id'];
    			}
    	    }
			//21-04-2021 exp_imp_upd end
		
			$date_release = $this->getCell($data,$i,$j++);
			$date_release = ((is_string($date_release)) && (strlen($date_release)>0)) ? $date_release : "NOW()";
			$release_status_id = $this->getCell($data,$i,$j++,'0'); 
			$distributor_status_id = $this->getCell($data,$i,$j++,'0');
			$video = $this->getCell($data,$i,$j++,'');
			$product_rating = $this->getCell($data,$i,$j++,'');
			$product = array();
			...
			$product['date_release'] = $date_release;
			$product['release_status_id'] = $release_status_id;
			$product['distributor_status_id'] = $distributor_status_id;
			$product['video'] = $video;
...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array_merge( $expected_heading, array( "meta_description", "meta_keywords", "stock_status_id", "store_ids", "layout", "related_ids", "tags", "sort_order", "subtract", "minimum", "date_release", "release_status_id", "distributor_status_id", "video", "product_rating") );
		if ($exist_meta_title) {
	...
	protected function clearCache() {
		$this->cache->delete('*');
			...
			public function upload( $filename, $incremental=false, $exclude_qsd ) {
		...
		try {
			...
			$this->uploadProducts( $reader, $incremental, $available_product_ids, $exclude_qsd );
	...
	protected function getProducts( &$languages, $default_language_id, $product_fields, $exist_meta_title, $exist_seo_url_table, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		...
		$sql .= "  p.minimum, ";
		$sql .= "  p.date_release, ";
		$sql .= "  p.release_status_id, p.distributor_status_id, p.video,  ";
		$sql .= "  p.product_rating, ";
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('date_release'),19)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('release_status_id'),3)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('distributor_status_id'),3)+1);
			$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('video')+4,32)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('product_rating'),10)+1);		
		...
		$styles[$j] = &$text_format;
		$data[$j++] = 'date_release';
		$data[$j++] = 'release_status_id';
		$data[$j++] = 'distributor_status_id';
		$data[$j++] = 'video';
		$data[$j++] = 'product_rating';
		$worksheet->getRowDimension($i)->setRowHeight(30);
		$this->setCellRow( $worksheet, $i, $data, $box_format );
		...
		foreach ($products as $row) {
			...
			$data[$j++] = $row['minimum'];
			$data[$j++] = $row['date_release'];
			$data[$j++] = $row['release_status_id'];
			$data[$j++] = $row['distributor_status_id'];
			$data[$j++] = $row['video'];
			$data[$j++] = $row['product_rating'];	
			$this->setCellRow( $worksheet, $i, $data, $this->null_array, $styles );
			$i += 1;
			$j = 1;
		}
	...
	function populateDeliveryWorksheet ( &$worksheet, $language_id, &$price_format, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
		// Set the column widths
		$j = 1;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('Order Numbers')+5);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('Order Status')+5);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('quantity')+5);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('Product')+5);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('ID'),10)+5);
	...
	$data['delivery'] = array();
	$results = $this->model_sale_delivery->getdeliveryColumn('');
	foreach ($results as $result) {
		...
		$delivery_status_name = null;
		 ...
		 foreach ($delivery_statuses as $deliverys) {
            if ($deliverys['delivery_status_id'] === $result['delivery_status_id']) {
                $worksheet->getRowDimension($i)->setRowHeight(13);
                $data = array();  // Reset data array for the current row
                
                // Get order links and flatten the array to a string
                $order_links = array();
                ...
                foreach ($order_ids as $order_id_product) {
                    $order_links[] = $order_id_product;
                }
                // Flatten the array into a string with comma separation for Excel
                $data[$j++] = implode(', ', $order_links);
                $data[$j++] = $deliverys['name'];
                $data[$j++] = $result['name'];
                $data[$j++] = $result['model'];
                $data[$j++] = $result['quantity'];
                
                // Set the row data in the worksheet
                $this->setCellRow($worksheet, $i, $data, $this->null_array, $styles);
		...
		try {
			...
			switch ($export_type) {
				...
				case 'p':
					...
					// creating the delivery worksheet
                    $workbook->createSheet();
                    $workbook->setActiveSheetIndex($worksheet_index++);
                    $worksheet = $workbook->getActiveSheet();
                    $worksheet->setTitle( 'Delivery' );
                    $this->populateDeliveryWorksheet( $worksheet, $default_language_id, $price_format, $box_format, $text_format, $min_id, $max_id );
                    $worksheet->freezePaneByColumnAndRow( 2, 2 );
