...
class ControllerSaleReturn extends Controller {
	...
	protected function getForm() {
		...
		$return_cost = $this->model_sale_return->getReturnCost($this->request->get['return_id']);

    	if (isset($this->request->post['cost'])) {
      		$data['cost'] = $this->request->post['cost'];
    	} elseif (!empty($return_info)) { 
    		if ($return_cost) {
    			$data['cost'] = $return_cost['cost'];
    		} else {
    			$data['cost'] = '0.0000';	
			}
		} else {
	...
	public function addHistory() {
		...
		if (!$this->user->hasPermission('modify', 'sale/return')) {
            ...
            $this->model_sale_return->addReturnHistory($this->request->get['return_id'], $this->request->post['return_status_id'], $this->request->post['comment'], $notify);
