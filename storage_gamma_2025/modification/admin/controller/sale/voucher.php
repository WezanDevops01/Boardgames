...
class ControllerSaleVoucher extends Controller {
	...
	public function send() {
		...
		if (!$json) {
			...
			//HUNTBEE CUSTOMER VOUCHER EMAIL CODE STARTS
			$this->load->model('extension/hbapps/email_builder');
			...
			if ($order_info) {
				...
				$language_id = (int)$this->config->get('config_language_id');
				$voucher_amount = $this->currency->format($voucher_info['amount'], $this->config->get('config_currency'));
				$store_id = 0;
			}
			$voucher_image_src = $data['image'];
			$voucher_image = '<img src="'.$voucher_image_src.'" alt="voucher image" />';	
			
			$template_id  = 0;
			$this->load->model('setting/setting');
			$extn_info = $this->model_setting_setting->getSetting('hb_uet', $store_id);
			if (isset($extn_info['hb_uet_cust_voucher_'.$language_id])) {
				$template_id = $extn_info['hb_uet_cust_voucher_'.$language_id];
			}
		
			if ($template_id > 0){
				$template_data = $this->model_extension_hbapps_email_builder->builtTemplate($template_id);
				$template_subject = $template_data['email_options']['email_subject'];
				$template_body = $template_data['email_content'];
				
				$template_body = str_replace('{from_name}',$voucher_info['from_name'],$template_body);
				$template_body = str_replace('{voucher_amount}',$voucher_amount,$template_body);
				$template_body = str_replace('{voucher_code}',$voucher_info['code'],$template_body);
				$template_body = str_replace('{voucher_message}',nl2br($voucher_info['message']),$template_body);
				$template_body = str_replace('{voucher_image_src}',$voucher_image_src,$template_body);
				$template_body = str_replace('{voucher_image}',$voucher_image,$template_body);
				
				$template_subject = str_replace('{from_name}',$voucher_info['from_name'],$template_subject);
			...
			//execute default code for customer voucher from admin
			...
			} // HB CODE end default code for customer voucher from admin
