...
'stock_status_id'         => (isset($query->row['stock_status_id']))?$query->row['stock_status_id']:0,
				...
				'product_rating'   =>$query->row['product_rating'],
			);
		} else {
			return false;
		}
	...
	public function getProductSpecials($data = array()) {
     ...
     //non exist  discount product id from both special and discount offer table
    	public function getProductSpecials_All($data = array()) {
		$sql = "SELECT ps.product_id
					FROM oc_product_special ps
					LEFT JOIN oc_discountoffer do ON ps.product_id = do.product_id
					WHERE do.product_id IS NULL ";	
	 ...
	 //daily dicount product
    	public function getProductSpecials_Daily($data = array()) {
		$sql = "SELECT product_id FROM `" . DB_PREFIX . "discountoffer` WHERE date_interval=1";	
	 ...
	 //weekly dicount product
    	public function getProductSpecials_Weekly($data = array()) {
		$sql = "SELECT product_id FROM `" . DB_PREFIX . "discountoffer` WHERE date_interval=7";	
	...
	public function getInformation($information_id) {
        // Get the information descriptions (assuming there may be multiple languages)
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information_description WHERE information_id = '" . (int)$information_id . "'");
        
        $information_descriptions = array();
        foreach ($query->rows as $result) {
            // Key by language_id (adjust if needed)
            $information_descriptions[$result['language_id']] = array(
                'instock_description'    => $result['instock_description'],
                'preorder_description'   => $result['preorder_description'],
                'coming_soon_description'=> $result['coming_soon_description'],
                'reordered_description'  => $result['reordered_description'],
                'restocked_description'  => $result['restocked_description'],
                'new_description'    => $result['new_description'],
                'like_new_description'   => $result['like_new_description'],
                'very_good_description'=> $result['very_good_description'],
                'good_description'  => $result['good_description'],
                'acceptable_description'  => $result['acceptable_description'],
                
            );
        }
        
        // Get the status from the information table
        $query2 = $this->db->query("SELECT status FROM " . DB_PREFIX . "information WHERE information_id = '" . (int)$information_id . "'");
        $status = ($query2->num_rows ? $query2->row['status'] : 0);
        
        return array(
            'status' => $status,
            'information_descriptions' => $information_descriptions
        );
    }
