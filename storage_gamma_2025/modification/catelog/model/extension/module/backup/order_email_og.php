...
class ModelExtensionModuleOrderEmail extends Controller {	
	...
	// Determine address format
        if ($order_info['payment_address_format']) {
            ...
            // Default format (add {apartment_num} if it exists later)
        ...
        // ✅ Add dynamic custom field support
        if (!empty($order_info['payment_custom_field']) && is_array($order_info['payment_custom_field'])) {
            // Load custom field info from the database to map IDs → codes
            $custom_fields = $this->model_account_custom_field->getCustomFields();
            $custom_field_map = array();
        
            foreach ($custom_fields as $field) {
                $custom_field_map[$field['custom_field_id']] = $field['name'];
                // Example: [1 => 'apartment_num']
            }
        
            foreach ($order_info['payment_custom_field'] as $custom_field_id => $custom_value) {
                // Try to get the custom field code or name
                $code = isset($custom_field_map[$custom_field_id])
                    ? preg_replace('/\s+/', '_', strtolower($custom_field_map[$custom_field_id]))
                    : $custom_field_id;
        
                // Support both placeholders: numeric {1} and code {apartment_num}
        ...
        // Generate HTML-friendly formatted address
