...
class ModelExtensionModuleOrderEmail extends Controller {	
	public function compose($data){
		...
		// ===================================================
// FORCE HOLD STATUS IN EMAIL BASED ON SHIPPING METHOD
// ===================================================
if (
    $email_type === 'order_confirmation' &&
    !empty($order_info['shipping_code']) &&
    $order_info['shipping_code'] === 'xshippingpro.xshippingpro27' &&
    (int)$order_info['order_status_id'] === 0
) {
    $order_status_id = 27;
}
...
// fetch all linked orders already processed (status = 2)
$query = $this->db->query("
    SELECT linked_order
...
if ($query->num_rows) {
    $linked_orders_block .= '<b>Linked Orders:</b> ';
    $ids = [];
    foreach ($query->rows as $row) {
        $ids[] = '#' . $row['linked_order'];
    }
    $linked_orders_block .= implode(', ', $ids);
}

$data['linked_orders_block'] = $linked_orders_block;
...
/*// Add custom merge fields into template replacements
$data['linked_order_id'] = $data['linked_order_id'];
$data['linked_from_order_id'] = $data['linked_from_order_id'];
$data['linked_orders_block'] = $data['linked_orders_block'];*/
...
// --- Custom Linked Order Merge Tags ---
$email_body = str_replace('{linked_order_id}', $data['linked_order_id'], $email_body);
$email_body = str_replace('{linked_from_order_id}', $data['linked_from_order_id'], $email_body);
