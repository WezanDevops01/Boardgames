...
class ControllerMailOrder extends Controller {
	public function index(&$route, &$args) {
		...
		if ($order_info) {
            ...
            public function hb_orderemail(&$route, &$args) {
        		if (isset($args[0])) {
        			$order_id = $args[0];
        		} else {
        			$order_id = 0;
        		...
        		if ($order_info) {
					$this->load->model('extension/module/order_email');
            		if (!$order_info['order_status_id'] && $order_status_id) {
        			   $this->load->model('extension/module/order_email');
						$email_data = array(
							'order_id' 			=> $order_id,
							...
							'order_history_id'	=> false,
							'notify'			=> true,
							'email_type'		=> 'order_confirmation'
						);
						$huntbee_order_confirmation = $this->model_extension_module_order_email->compose($email_data);
						if ($huntbee_order_confirmation == false) {
							$this->add($order_info, $order_status_id, $comment, $notify);
						}
						...
						$email_data = array(
							...
							'admin_comment'		=> $comment,
							'notify'			=> true,
							'email_type'		=> 'order_alert'
						);
						$huntbee_order_alert = $this->model_extension_module_order_email->compose($email_data);
						...
						$huntbee_order_update = $this->model_extension_module_order_email->compose($email_data);
						if ($huntbee_order_update == false && $notify) {
							$this->edit($order_info, $order_status_id, $comment, $notify);
						}
        			}
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['text_order_detail'] = $language->get('text_order_detail');
				...
				$data['text_preorder'] = $language->get('text_preorder');			
		...
		$data['customer_id'] = $order_info['customer_id'];
                ...
                $data['link'] = $this->model_extension_module_order_view->getDirectLink($order_info['order_id']);
	 ...
	 if ($order_info['shipping_address_format']) {
		...
		$mail->smtp_timeout = $this->config->get('config_mail_smtp_timeout');
                ...
                $this->load->model('setting/setting');
                $pdf_config = $this->model_setting_setting->getSetting('hb_pdf', $order_info['store_id']);
                ...
                if (in_array($order_status_id, $hb_pdf_iv_trigger_status)) {
                    $pdf_invoice =  $this->model_extension_module_hb_pdfinvoice->createPdf('iv', $order_info['order_id'], false, $pdf_auth, false);
		...
		$mail->send();
                ...
                if ((isset($pdf_invoice)) && isset($hb_pdf_keep_backup) && ($hb_pdf_keep_backup == false) && (file_exists($pdf_invoice)) ) {
					unlink($pdf_invoice);
				}
