...
/* AbandonedCarts - End */
	...
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
		   ...
		   // Add Gift Vouchers as pseudo-products
			// Convert voucher to product only when NO normal products
if (empty($products) && !empty($this->session->data['vouchers'])) {
    foreach ($this->session->data['vouchers'] as $key => $voucher) {

        // --- Step 1: Create dynamic product if not already created ---
        if (!isset($voucher['dynamic_product_id'])) {

            // Insert into oc_product
            $this->db->query("
                INSERT INTO `" . DB_PREFIX . "product`
                SET 
                    model = 'GV-" . $this->db->escape($key) . "',
                    price = '" . (float)$voucher['amount'] . "',
                    quantity = 1,
                    stock_status_id = 7,
                    shipping = 0,
                    date_added = NOW(),
            ...
            $this->db->query("
                ...
                SET 
                    ...
                    language_id = 1,
                    name = '" . $this->db->escape($voucher['description']) . "',
                    description = 'Gift Voucher Auto Created',
                    meta_title = '" . $this->db->escape($voucher['description']) . "'
            ");
            ...
            // Save product_id in voucher session so it won't create again
            $this->session->data['vouchers'][$key]['dynamic_product_id'] = $product_id;

        } else {
            $product_id = $voucher['dynamic_product_id'];
        }
	...
	if (!$already_in_cart) {
        ...
        // --- Step 2: Add voucher as real product to $products array ---
       /* $products[] = [
            'cart_id'         => $key,
	...
	$products = $this->cart->getProducts();
    // Hide voucher section to avoid duplication
    $data['vouchers'] = [];
}
/*echo"<pre>";
print_r($products);
