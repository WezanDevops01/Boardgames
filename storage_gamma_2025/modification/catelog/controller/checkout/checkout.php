...
class ControllerCheckoutCheckout extends Controller {
		...
		$json = $this->model_extension_shipping_wk_custom_shipping->getsellershipping($this->request->post);
	...
	public function index() {
          ...
          if ((count($seller) > 1) && $this->config->get('module_wk_split_cart_status') && !isset($this->request->get['wk']) ) {
		...
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$this->response->redirect($this->url->link('checkout/cart'));
				...
				if (
					...
					(
						(
							...
							&& (int)$this->config->get('marketplace_min_cart_value') > $this->cart->getSubTotal()
						)
						||
						$this->model_account_customerpartner->getProductRestriction()
					)
				) {
					$this->response->redirect($this->url->link('checkout/cart'));
				}
                ...
                if ($this->config->get('module_mp_store_pickup_status') && isset($this->session->data['pickup_points']) && $this->session->data['pickup_points'] && $this->load->controller('mp_store_pickup/mp_store_pickup/checkPickupPointStock')) {
