...
class ControllerJournal3Products extends ModuleController {
	...
	private function parseProducts($results) {
		...
		foreach ($results as $result) {
			...
			$product_attributes = $this->model_journal3_product->getProductAttributes($result['product_id']);
          
               $player = $length = $product_rating = $difficulty = '-'; // Initialize variables

                foreach ($product_attributes as $attribute) {
                    if ($attribute['attribute_id'] == '501') {
                      $player = trim(str_ireplace('players', '', $attribute['text']));
                      if($player == '? - ?'){
                                    $player = "-";
                                  }
                    } elseif ($attribute['attribute_id'] == '503') {
                           // Assuming the format is "min-maxmins", extract the max value
                            $parts = explode('-', $attribute['text']);
                            if (count($parts) === 2) {
                    ...
                    } elseif ($attribute['attribute_id'] == '511') {
                         if (!empty($attribute['text'])) {
                        $rating_parts = explode('/', $attribute['text']);
                        $product_rating = trim($rating_parts[0]);
                         } else {
                            $product_rating = '-';
                        }
                    } elseif ($attribute['attribute_id'] == '510') {
                       if (!empty($attribute['text'])) {
                            $diff_parts = explode('/', $attribute['text']);
			...
			$result['stock_status_id'] = isset($result['stock_status_id'])?$result['stock_status_id']:'0'; //added as j3 not getting this value
