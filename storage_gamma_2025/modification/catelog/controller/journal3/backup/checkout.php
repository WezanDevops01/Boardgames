...
class ControllerJournal3Checkout extends \Journal3\Opencart\Controller {
	...
	public function index() {
	    $this->journal3->document->addScript('catalog/view/theme/journal3/js/checkout-combine.js');
      ...
      if ($this->config->get('shipping_wk_custom_shipping_status')) {
          $this->load->language('extension/shipping/wk_custom_shipping');
              $custom_shipping['title'] = $this->config->get('shipping_wk_custom_shipping_title');
          if ($this->config->get('shipping_wk_custom_shipping_priority_status')) {
            ...
            $custom_shipping['text_priority']        = $this->language->get('text_priority');
            $custom_shipping['entry_high']           = $this->language->get('entry_high');
            $custom_shipping['entry_medium']         = $this->language->get('entry_medium');
            $custom_shipping['entry_low']            = $this->language->get('entry_low');
            $custom_shipping['text_seller_name']     = $this->language->get('text_seller_name');
            $custom_shipping['text_seller_shipping'] = $this->language->get('text_seller_shipping');
            $custom_shipping['text_day']             = $this->language->get('text_day');
            $custom_shipping['text_duration']        = $this->language->get('text_duration');
            $custom_shipping['text_charges']         = $this->language->get('text_charges');
            $custom_shipping['text_event_amount']    = $this->language->get('text_event_amount');
            $custom_shipping['priority_shipping']    = isset($priority_shipping) ? $priority_shipping : array();
            
          }

          if (isset($this->session->data['shipping_methods']) && isset($this->session->data['shipping_methods']['wk_custom_shipping']) && isset($this->session->data['seller_custom_shipping']) && $this->session->data['seller_custom_shipping'] && $this->config->get('shipping_wk_custom_shipping_seller_status')) {
            $custom_shipping['seller_custom_shipping'] = $this->session->data['seller_custom_shipping'];
            $custom_shipping['text_seller_name'] = $this->language->get('text_seller_name');
            $custom_shipping['text_seller_shipping'] = $this->language->get('text_seller_shipping');
            $custom_shipping['text_event_amount'] = $this->language->get('text_event_amount');
            $custom_shipping['event_shipping_amount'] = isset($this->session->data['event_shipping_amount']) ? $this->session->data['event_shipping_amount'] : 0;
          }
        }
      ...
      'custom_shipping' => isset($custom_shipping) ? $custom_shipping : array(),
		...
		// BEGIN: Detect mixed cart (preorder + non-preorder)
        $preorder_category_id = 53; // Preorder category ID
        $has_preorder = false;
        ...
        foreach ($this->cart->getProducts() as $product) {
            ...
            $category_ids = array_column($categories, 'category_id');
                ...
                if (in_array($preorder_category_id, $category_ids)) {
					$has_preorder = true;
					$preorder_count += $product['quantity'];
				} else {
					$has_non_preorder = true;
				}
            // Duplicate cart in cart
        ...
        $is_mixed_cart = $has_preorder;
         // END: Detect mixed cart
	...
	public function save() {
		...
		if ($this->config->get($this->journal3->isOC2() ? 'reward_status' : 'total_reward_status') && $this->request->post['reward']) {
            ...
            $isl_coupon = isset($this->request->post['coupon']) ? $this->request->post['coupon'] : '';

            if (!empty($this->session->data['coupon_promotions'])) {
                $this->session->data['coupon'] = $this->session->data['coupon_promotions'];
            }

            if (!empty($error['coupon']) && $isl_coupon) {
                ...
                $islpr_coupon = $this->{$this->isl_promotions['model']}->couponCheck($isl_coupon);
                ...
                if ($islpr_coupon['active']) {
                    ...
                    if ($islpr_coupon['valid']) {
                        $this->session->data['coupon_promotions'] = $isl_coupon;
                        $this->session->data['coupon'] = $isl_coupon;
                    } else {
                       $error['coupon'] = sprintf($this->language->get('text_coupon_error'), $coupon, $islpr_coupon['url'], $islpr_coupon['title']);
                    }
                }
            }
		...
		if ($this->checkCart()) {
			...
			if (Arr::get($this->request->get, 'confirm') === 'true') {
				$order = $this->request->post['order_data'];
	/*echo "<pre style='background:#000;color:#0f0;padding:20px;font-size:14px;'>";
    echo "===== SCREEN DEBUG: REQUEST POST =====\n";
    //print_r($this->request->post);
    echo "Shipping Method: " . ($order['shipping_method'] ?? 'N/A') . "\n";
    echo "Shipping Code:   " . ($order['shipping_code'] ?? 'N/A') . "\n";
    //echo "\n===== SCREEN DEBUG: ORDER DATA =====\n";
    //print_r($data);

    //echo "\n===== SCREEN DEBUG: SESSION DATA =====\n";
    ...
    exit;  */
                ...
                if ($duplicate_item) {
                    $json['duplicate_item'] = "You have duplicate items in your cart, please verify before confirming.";
                }
				...
				if ($agree && !$this->validateInformation('agree')) {
					...
					$error['agree'] = "You must agree to the Terms & Conditions and Privacy Policy.";
				}
		...
		$json['error'] = $error ? $error : null;
				...
				if ($this->config->get('hb_cart_status')) {
    				if (isset($json['order_data']['email']) and !empty($json['order_data']['email'])){
                		$cookie_expiration_time = time() + (90 * 24 * 60 * 60);  // for 3 month
            			$cart_guest = $this->db->query("SELECT * FROM `".DB_PREFIX."cart_guest` WHERE session_id = '".$this->db->escape($this->session->getId())."' LIMIT 1");
            			if ($cart_guest->num_rows > 0 ) {
            				$this->db->query("UPDATE `".DB_PREFIX."cart_guest` SET guest_details = '".$this->db->escape(json_encode($json['order_data']))."' WHERE session_id = '". $this->db->escape($this->session->getId()) ."'");
            			}else{
            				$this->db->query("INSERT INTO `".DB_PREFIX."cart_guest` (`session_id`, `guest_details`,`store_id`) VALUES ('".$this->db->escape($this->session->getId())."', '".$this->db->escape(json_encode($json['order_data']))."', '" . (int)$this->config->get('config_store_id') . "')");
            			}
            				
            			setcookie("hb_email", $json['order_data']['email'], $cookie_expiration_time, "", "", "", "true");    
            		}
    			}
				//HB ABANDONED CART
	...
	private function products() {
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			$result[] = array(
				...
				'text_preorder'=> $this->language->get('text_preorder'),
				'option'     => $option_data,
				'recurring'  => $recurring,
...
*/
        if ($this->customer->isLogged()) {
            ...
            // Loop through addresses to merge custom_field with address_1
            foreach ($addresses as &$address) {
                if (isset($address['custom_field'][1])) {
                    $address['address_1'] = $address['custom_field'][1] . ' ' . $address['address_1'];
                }
            }
            unset($address); // Unset reference variable to avoid potential bugs
