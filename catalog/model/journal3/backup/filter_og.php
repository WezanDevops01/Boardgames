...
/*	private function addFilters($filter_data, $query = null) {
    ...
    // FIXED CATEGORY CONDITION
    if ($query !== 'category' && (
            Arr::get($filter_data, 'categories') ||
            (isset($filter_data['filter_category_id']) && (int)$filter_data['filter_category_id'] > 0)
        )) {
    ...
    // FIXED CATEGORY CONDITION (WHERE BLOCK)
    if ($query !== 'category' && (
            Arr::get($filter_data, 'categories') ||
            (isset($filter_data['filter_category_id']) && (int)$filter_data['filter_category_id'] > 0)
        )) {
