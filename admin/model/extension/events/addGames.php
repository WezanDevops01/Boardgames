...
class ModelExtensionEventsAddGames extends Model {
     public function addGames($games) {
        ...
        foreach ($games as $game) {
            ...
            ");
                    // 🔹 NEW CODE: Insert/Update video in oc_event_product_details
        $eventQuery = $this->db->query("
            SELECT product_id 
            FROM " . DB_PREFIX . "event_product_details 
            WHERE product_id = '" . (int)$game['product_id'] . "'
        ");

        if ($eventQuery->num_rows) {
            // Update existing row
            $this->db->query("
                UPDATE " . DB_PREFIX . "event_product_details 
                SET video = '" . $this->db->escape($game['product_video']) . "'
                WHERE product_id = '" . (int)$game['product_id'] . "'
            ");
        } else {
            // Insert new row
            $this->db->query("
                ...
                SET product_id = '" . (int)$game['product_id'] . "',
                    video      = '" . $this->db->escape($game['product_video']) . "'
            ");
        }
