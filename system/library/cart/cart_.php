...
// -------------------------------------------------------------
// GIFT VOUCHER PRICE OVERRIDE
// If product_id = 7766, replace product price with voucher amount
// -------------------------------------------------------------
if ($cart['product_id'] == 7766) {
    if (!empty($option_data)) {
        foreach ($option_data as $opt) {
            if (strtolower($opt['name']) == 'amount' || strtolower($opt['name']) == 'voucher amount') {
                $voucher_amount = (float)$opt['value'];
                $price = $voucher_amount;
                $option_price = 0; // no option math for vouchers
            }
        }
    }
}
